...
>(
  ...
  copyValue: ({ rowData, rowIndex }: any) =>
      column.copyValue?.({ rowData: getValueAtPath(rowData, key), rowIndex }) ?? null,
  deleteValue: ({ rowData, rowIndex }: any) => {
    setValueAtPath(rowData, key, column.deleteValue?.({ rowData, rowIndex }) ?? null)
    return {
    ...rowData,
    }
  },
  pasteValue: ({ rowData, value, rowIndex }: any) => {
    setValueAtPath(rowData, key, column.pasteValue?.({ rowData: getValueAtPath(rowData, key), value, rowIndex }))
    return {
    ...rowData,
  }},
  disabled:
    ...
    typeof column.cellClassName === 'function'
      ? ({ rowData, rowIndex, columnId }: any) => {
          return typeof column.cellClassName === 'function'
            ? column.cellClassName({ rowData: getValueAtPath(rowData, key), rowIndex, columnId })
            : column.cellClassName ?? undefined
        }
      : column.cellClassName,
  isCellEmpty: ({ rowData, rowIndex }: any) =>
    column.isCellEmpty?.({ rowData: getValueAtPath(rowData, key), rowIndex }) ?? false,
} as any)
